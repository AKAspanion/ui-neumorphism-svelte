...
type ThemeObjectType = Partial<typeof defaultRootCSSVariables>;
 ...
 * @param {*} value
 */
export const setCSSVariable = (element: HTMLElement, variable: string, value: string) => {
	if (element && value) {
 ...
 * @param {*} themeObject
 */
export const overrideThemeVariables = (themeObject: ThemeObjectType) => {
	const root = document.querySelector(':root') as HTMLElement;
	const themeVariables = Object.keys(themeObject);
	if (root && themeObject) {
		themeVariables.forEach((themeVar) => {
			const varValue = themeObject[themeVar as keyof ThemeObjectType];
 ...
 * @param {*} element
 */
export const detectElementInDOM = (path: Element[] = [], element = 'null') => {
	return path
...
export const findClickInside = (event: Event, node: Element) => {
	...
	try {
		do {
			...
			currentNode = <HTMLElement>currentNode?.parentNode;
		} while (currentNode);
		// click is outside
		return false;
	} catch (err) {
		throw new Error('Failed to find click inside');
	}
};
